                  ...
                  int32_t          resize_height)
{
   ...
   //       int offset_p = index_y * src_stride + (index_x << 1);
   //       uint32_t index_mod_p = (index_x & 1) * 2;
   //       printf("offset_p: %d, index_mod_p: %d ",offset_p,index_mod_p);
   //       uint16_t offset = resize_map[(jj*2)+ii*resize_width*2];
   //       uint16_t index_mod = resize_map[(jj*2)+1+ii*resize_width*2];
   //       printf("offset: %d, index_mod: %d ",offset,index_mod);
   ...
   {
      /* code */
      uint32_t  *p_resize_map = resize_map + i * resize_width * 2;
      ...
      for (int32_t j = 0; j < resize_width; j++)
      {
         ...
         uint32_t offset = p_resize_map[(j << 1)];
         uint32_t index_mod = p_resize_map[(j << 1)+1];
         ...
         // uint8_t resize_y = *(src + offset + 1);
         // uint8_t resize_u = *(src + offset - index_mod);
         // uint8_t resize_v = *(src + offset + 2 - index_mod);
...
int32_t  InitResizeScaler(int32_t src_width, int32_t src_height, int32_t resize_width, int32_t resize_height, uint32_t *resize_scaler)
{
   float h_scale = src_width*1.0/resize_width;
   float v_scale = src_height*1.0/resize_height;
   int32_t chn = 2;
   ...
   {
      uint32_t   *p_dst  = resize_scaler + (ii * resize_width) * chn;
      for (int jj = 0; jj < resize_width; jj++)
      {
         int index_x = round(jj*h_scale);
         int index_y = round(ii*v_scale);

         uint32_t offset = index_y * src_stride + (index_x << 1);
         uint32_t index_mod = (index_x & 1) * 2;

         resize_scaler[jj*chn + 0 + (ii * resize_width) * chn] = offset;
         resize_scaler[jj*chn + 1 + (ii * resize_width) * chn] = index_mod;
...
int main(int argc, char* argv[])
{
   ...
   uint32_t *resize_scaler = (uint32_t *)malloc(resize_width * resize_height * sizeof(uint32_t) * 2);
