 ...
 * Author : xu_wenying
 * Version: v1.0.0
 * Modification: Created at 2023.01.05
 *
 * Description: This is a example for coding.
 *
 *******************************************************************************/
                  ...
                  int32_t          resize_height)
{
   ...
   {
      /* code */
      // uint32_t  *p_map = map + i * map_width * 2;
      uint8_t   *p_dst  = dst + (i * resize_width) * chn;
      for (int32_t j = 0; j < resize_width; j++)
      {
         /* code */

         int index_x = round(j*h_scale);
         int index_y = round(i*v_scale);
         
         int offset = index_x * 2 + index_y * src_stride;
...
int main(int argc, char* argv[])
{
   ...
   {
      TIME_START(t0);
      int32_t rettt = YuvResize2Rgb(pSrc_yuv, src_width, src_height, pDst_rgb, resize_width, resize_height);
